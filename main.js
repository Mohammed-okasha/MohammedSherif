...
// projectsSlides
const  projectsSlides = document.querySelector(".projects-slides .swiper-wrapper");
...
function fetchProjects(apiurl) {
    ...
    xhr.addEventListener("load", function() {
        ...
        if (this.readyState === 4 && this.status === 200) {
            ...
            categories = [ ...new Set(["all", ...projects.map(project => project.category)])];
...
function displayProjects(projects) {
    ...
    projects.forEach(project => {
        const { id, title, img, description: desc, link, category: cat} = project;
